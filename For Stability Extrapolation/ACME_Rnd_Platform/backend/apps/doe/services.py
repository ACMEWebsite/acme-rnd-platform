...
def generate_design(factors):
    trials = []
    for index, levels in enumerate(
        product(*[(factor["low"], factor["high"]) for factor in factors]), start=1
    ):
        trials.append({
            "trial": index,
...
def rank_trials(trials, goals):
    ...
    for trial in trials:
        ...
        valid = True
        for goal in goals:
            value = trial.get(goal.get("response"))
            try:
                value = float(value)
            ...
            weight = max(float(goal.get("weight", 1)), 0)
            direction = goal.get("direction", "maximize")
            component = (
                value if direction == "maximize"
                else -value if direction == "minimize"
                else -abs(value - target)
            )
            score += component * weight
        prepared.append({**trial, "optimization_score": round(score, 6), "complete": valid})
    return sorted(prepared, key=lambda row: row["optimization_score"], reverse=True)
